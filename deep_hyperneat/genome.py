...
import numpy as np
from deep_hyperneat.util import iteritems,iterkeys
...
class Genome():
    ...
    def save_to_json(self, filename):
        ...
        data = {
            "key": self.key,
            "connections": {str(k): self._serialize_connection(v) for k, v in self.connections.items()},
            "nodes": {str(k): self._serialize_node(v) for k, v in self.nodes.items()},
            ...
            "bias_keys": self.bias_keys,
            "cppn_tuples": self.cppn_tuples,
            "substrate": self.substrate,
            "num_depth": self.num_depth,
            "num_breadth": self.num_breadth,
            "_complexity": self._complexity
        }
    ...
    def _serialize_connection(self, connection):
        """Helper method to serialize ConnectionGene objects"""
        return {
            "key": connection.key,
    ...
    def _serialize_node(self, node):
        """Helper method to serialize NodeGene objects"""
        return {
            "key": node.key,
            "type": node.type,
            "activation": node.activation.__name__ if callable(node.activation) else str(node.activation),
            "bias": node.bias,
            "response": node.response,
            "cppn_tuple": node.cppn_tuple
        }
    ...
    def load_from_json(cls, filename):
        ...
        genome = cls(data["key"])
        genome.connections = {eval(k): cls._deserialize_connection(v) for k, v in data["connections"].items()}
        genome.nodes = {eval(k): cls._deserialize_node(v) for k, v in data["nodes"].items()}
        genome.fitness = data["fitness"]
        genome.num_inputs = data["num_inputs"]
        genome.num_outputs = data["num_outputs"]
        genome.num_layers = data["num_layers"]
        genome.input_keys = data["input_keys"]
        genome.output_keys = data["output_keys"]
        genome.bias_keys = data["bias_keys"]
        genome.cppn_tuples = data["cppn_tuples"]
        genome.substrate = data["substrate"]
        genome.num_depth = data["num_depth"]
        genome.num_breadth = data["num_breadth"]
        genome._complexity = data["_complexity"]

        return genome

    @staticmethod
    def _deserialize_connection(data):
        """Helper method to deserialize ConnectionGene objects"""
        return ConnectionGene(data["key"], data["weight"])

    @staticmethod
    def _deserialize_node(data):
        """Helper method to deserialize NodeGene objects"""
        activation_func = getattr(ActivationFunctionSet(), data["activation"])
        node = NodeGene(data["key"], data["type"], activation_func, data["cppn_tuple"])
        node.bias = data["bias"]
    ...
    def create_node(self,node_type='hidden',mapping_tuple=None,key=None):
        ...
        new_node_key = self.get_new_node_key() if key is None else key
